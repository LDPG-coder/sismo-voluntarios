...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(new URL(`${basePath}/login?error=${error}`, WEB_ORIGIN));
  }

  const code = searchParams.get("code");
  if (!code) {
    return NextResponse.redirect(new URL(`${basePath}/login?error=sep_missing_code`, WEB_ORIGIN));
  }
  ...
  if (!user) {
    return NextResponse.redirect(new URL(`${basePath}/login?error=sep_exchange`, WEB_ORIGIN));
  }
  if (user.status === "suspended") {
    return NextResponse.redirect(new URL(`${basePath}/login?error=suspended`, WEB_ORIGIN));
  }
  ...
  const redirectUrl =
    user.role === "admin" ? `${WEB_ORIGIN}${basePath}/admin` : `${WEB_ORIGIN}${basePath}/voluntarios`;
