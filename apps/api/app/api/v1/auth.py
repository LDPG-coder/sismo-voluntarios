...
class _ExchangeBody(BaseModel):
    code: str
    # PKCE verifier (S256) required when the code was issued with a challenge
    # (i.e. the SEP server-to-server login flow). Optional/ignored for the
    # legacy Google OAuth exchange, which issues codes without a challenge.
...
def auth_exchange(
    ...
    try:
        entry = oauth.consume_exchange_code(
            db,
            body.code,
            ttl=float(settings.oauth_exchange_ttl_seconds),
...
class _SepLoginBody(BaseModel):
    ...
    name: str | None = None
    role: str | None = None
    # PKCE S256 challenge (base64url(sha256(verifier))) generated by the SEP
    # backend. The matching verifier must later be presented when redeeming the
    # code at POST /api/v1/auth/exchange, binding the code to SEP.
...
def auth_sep_login(
    ...
    SEP proves its identity with `Authorization: Bearer <SISMO_SEP_LOGIN_TOKEN>`
    (falls back to the deprecated `SISMO_SEP_API_TOKEN`). The returned
    single-use, short-TTL code is then given to the browser, which redeems it at
    the web route `/auth/sep` (presenting the PKCE `code_verifier`) to obtain a
    normal session cookie — reusing the exact same mechanism as Google OAuth,
    but PKCE-bound so an intercepted code cannot be replayed.
    """
    sep_token = settings.sep_login_token or settings.sep_api_token
    if not sep_token:
        raise ApiError(
    ...
    if body.code_challenge and not _is_valid_pkce_challenge(body.code_challenge):
    ...
    code = oauth.issue_exchange_code(
        ...
        role=user.role,
        status=user.status,
        ttl=float(settings.sep_code_ttl_seconds),
...
def _is_valid_pkce_challenge(challenge: str) -> bool:
    # S256 challenge: base64url(sha256), 43 chars for a 32-byte digest.
    if not (43 <= len(challenge) <= 128):
        return False
    try:
        base64.urlsafe_b64decode(challenge + "=" * (-len(challenge) % 4))
    except Exception:
        return False
    return True
...
class _SepLogoutBody(BaseModel):
    sep_user_id: str


@router.post("/sep-logout", status_code=204, response_class=Response)
def auth_sep_logout(
    body: _SepLogoutBody,
    request: Request,
    settings: Annotated[Settings, Depends(get_settings)],
    ...
    """Coordinated logout: SEP calls this server-to-server when its own session
    ends, so SISMO terminates the SEP user's session too (not just the cookie).

    Same token as sep-login (`SISMO_SEP_LOGIN_TOKEN`). Idempotent: if the user
    was never provisioned in SISMO, we still return 204.
    """
    sep_token = settings.sep_login_token or settings.sep_api_token
    if not sep_token:
        raise ApiError(
    ...
    sep_user_id = body.sep_user_id.strip()
    if not sep_user_id:
        raise ApiError(ErrorCode.validation_missing_field, "sep_user_id is required")

    user = _user_by_sep_id(db, sep_user_id)
    if user is not None:
        from app.pipeline.session_store import revoke_user_sessions

        # Rotates the user's refresh-token family, invalidating every
        # outstanding refresh token at once; the short-lived access cookie is
        # also dropped client-side by SEP (see docs/SEP_INTEGRATION.md §2.5).
        revoke_user_sessions(str(user.id))
        _log.info("auth.sep.logout", sep_user_id=sep_user_id, user_id=str(user.id))
