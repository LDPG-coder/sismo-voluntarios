...
(async () => {
  ...
  const browser = await puppeteer.launch({ executablePath: '/usr/bin/chromium', headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--force-color-profile=srgb', '--hide-scrollbars'] });
  ...
  await page.evaluate(() => {
    ...
    const crear = links.find(a => a.textContent.includes('Crear'));
